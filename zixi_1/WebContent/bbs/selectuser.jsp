...
<div id="container">   
	...
	<caption>浏览用户</caption>
        ...
        <tbody>
				<c:forEach items="${sessionScope.alluser}"  var="user" varStatus="uuser">
				<tr>
				<td>${user.userId}</td>
				<td>${user.username}</td>
				<td>${user.password}</td>
				<td>${user.sex}</td>
				<td>${user.age}</td>
				<td><fmt:formatDate value="${user.birthd}"  pattern="yyyy年MM月dd日" /></td>
				<td>${user.integral}分</td>
				<td>
				<c:if test="${user.popedom==1}">普通用户</c:if>
				<c:if test="${user.popedom==2}">子版主</c:if>
				<c:if test="${user.popedom==3}">版主</c:if>
				<c:if test="${user.popedom==4}">管理员</c:if>
				</td>
				<td><a href="delete.action?userId=${user.userId}&d_delete=${user.d_delete}&popedom=${user.popedom}">
				<c:if test="${user.d_delete==0}">恢复</c:if>
				<c:if test="${user.d_delete==1}">封号</c:if>
				</a></td>
				<td>
				<c:if test="${user.d_delete==0}">违规已封</c:if>
				<c:if test="${user.d_delete==1}">正常用户</c:if>
				</td>
				</tr>
				</c:forEach>      
   </tbody>
